...
def display_data():
    ...
    cur.close()
    conn.close()
    return render_template("display_data.html", dataku=dataku,datapeta=datapeta,form_cari=form_cari)
...
def edit_data(id):
    ...
    if request.method=="POST":
       ...
       cur.execute("update hotosm_idn set osm_id=%s,name=%s,addrcity=%s,addrfull=%s,source=%s,operatorty=%s,amenity=%s where gid=%s",
                   ((xosm_id,xname,xaddrcity,xaddrfull,xsource,xoperatorty,xamenity,xgid)))
       cur.connection.commit()
       cur.close()
